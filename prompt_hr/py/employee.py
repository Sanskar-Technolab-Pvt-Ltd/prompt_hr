...
def set_employee_questionnaire(doc):
    ...
    for row in questionnaire:
        if not row.field_name:
            continue
        doc.append(
            "custom_pre_login_questionnaire_response",
            {
...
def on_update(doc, method):
    ...
    if (
        doc.custom_leave_policy_assignment_based_on_joining
        and doc.custom_leave_policy_assignment_based_on_custom_dates
    ):
        frappe.throw(
            _(
                "You cannot assign a leave policy based on both the Joining Date and Custom Dates at the same time."
            )
        )
    ...
    if is_leave_policy_assigned_from_employee_master:
        ...
        if doc.custom_assign_maternity_leave:
            leave_policy = frappe.db.get_single_value(
                "HR Settings", "custom_maternity_leave_policy"
            )
            if not leave_policy:
                frappe.msgprint("Please set Maternity Leave Policy in HR Settings")
            else:
                if doc.has_value_changed(
                    "custom_assign_maternity_leave"
                ) or doc.has_value_changed("custom_maternity_assignment_from_date"):
                    existing_assignment = frappe.db.exists(
                        "Leave Policy Assignment",
                        {
                            ...
                            "effective_from": [
                                "<=",
                                doc.custom_maternity_assignment_from_date,
                            ],
                            "effective_to": [
                                ">=",
                                doc.custom_maternity_assignment_from_date,
                    ...
                    if existing_assignment:
                        ...
                        assignment_doc.effective_from = (
                            doc.custom_maternity_assignment_from_date
                        )
                        assignment_doc.effective_to = add_years(
                            getdate(doc.custom_maternity_assignment_from_date), 60
                        )
                        assignment_doc.save()
        ...
        if doc.custom_assign_paternity_leave:
            leave_policy = frappe.db.get_single_value(
                "HR Settings", "custom_paternity_leave_policy"
            )
            if not leave_policy:
                frappe.msgprint("Please set Paternity Leave Policy in HR Settings")
            else:
                if doc.has_value_changed(
                    "custom_assign_paternity_leave"
                ) or doc.has_value_changed("custom_paternity_assignment_from_date"):
                    existing_assignment = frappe.db.exists(
                        "Leave Policy Assignment",
                        {
                            "employee": doc.name,
                            ...
                            "effective_from": [
                                "<=",
                                doc.custom_paternity_assignment_from_date,
                            ],
                            "effective_to": [
                                ">=",
                                doc.custom_paternity_assignment_from_date,
                    ...
                    if existing_assignment:
                        ...
                        assignment_doc.effective_to = add_years(
                            getdate(doc.custom_paternity_assignment_from_date), 60
                        )
                        assignment_doc.save()
                        assignment_doc.submit()
    ...
    if doc.custom_leave_policy and doc.has_value_changed("custom_leave_policy"):
        ...
        if doc.custom_leave_policy_assignment_based_on_joining and doc.flags.in_insert:
            ...
            doc.custom_leave_policy_assignment_based_on_custom_dates
            and doc.flags.in_insert
        ):
            ...
            if active_leave_period:
                if is_leave_policy_assigned_from_employee_master:
                    ...
                    if (
                        not doc.custom_leave_policy_assignment_based_on_custom_dates
                        or not doc.flags.in_insert
                    ):
                        if not old_doc:

                            create_leave_policy_assignment(
                                doc, 0, active_leave_period[0].get("name")
                            )
                            return
                        ...
                        if doc.custom_leave_policy_change_date:
                            confirmation_date = getdate(
                                doc.custom_leave_policy_change_date
                            )
                        ...
                        leave_allocation_calculation = (
                            calculate_leave_allocation_based_on_confirmation(
                                confirmation_date
                            )
                        )
                        ...
                        if leave_allocation_calculation:
                            ...
                            is_calculate_leave_allocation = (
                                leave_allocation_calculation.get(
                                    "calculate_leave_allocation"
                                )
                            )

                            months_count = leave_allocation_calculation.get(
                                "leave_months"
                            )

                        frappe.log_error(
                            "calculate_leave_allocation",
                            f" data {is_calculate_leave_allocation}",
                        )
                        ...
                        if old_leave_policy:
                            ...
                            old_leave_policy_leave_types = get_policy_leave_types(
                                old_leave_policy
                            )
                            new_leave_policy_leave_types = get_policy_leave_types(
                                new_leave_policy
                            )

                            old_earned_leave_types = (
                                old_leave_policy_leave_types.get("earned_leave_types")
                                or []
                            )
                            old_other_leave_types = (
                                old_leave_policy_leave_types.get("other_leave_types")
                                or []
                            )

                            new_earned_leave_types = new_leave_policy_leave_types.get(
                                "earned_leave_types"
                            )

                            leave_allocation_to_date = active_leave_period[0].get(
                                "to_date"
                            )
                            ...
                            for old_earned_leave_type in old_earned_leave_types:
                                ...
                                leave_allocation_exists = frappe.db.get_all(
                                    "Leave Allocation",
                                    {
                                        "employee": doc.name,
                                        "leave_policy": old_leave_policy,
                                        "leave_type": old_earned_leave_type.get(
                                            "leave_type"
                                        ),
                                        "from_date": ["<=", getdate()],
                                        "to_date": [">=", getdate()],
                                    },
                                    ["name", "leave_type", "to_date"],
                                    limit=1,
                                )

                                if leave_allocation_exists:

                                    leave_allocation_id = leave_allocation_exists[
                                        0
                                    ].get("name")
                                    ...
                                    if leave_allocation_id:
                                        frappe.db.set_value(
                                            ...
                                            leave_allocation_id,
                                            "to_date",
                                            add_to_date(confirmation_date, days=-1),
                                        )
                                        ...
                                        leave_ledger_entry_id = frappe.db.get_all(
                                            "Leave Ledger Entry",
                                            {
                                                ...
                                                "transaction_name": leave_allocation_id,
                                                "leave_type": old_earned_leave_type.get(
                                                    "leave_type"
                                                ),
                                            },
                                        ...
                                        if (
                                            ...
                                            and leave_ledger_entry_id[0].get("name")
                                        ):
                                            frappe.db.set_value(
                                                "Leave Ledger Entry",
                                                leave_ledger_entry_id[0].get("name"),
                                                "to_date",
                                                add_to_date(confirmation_date, days=-1),
                                            )

                                else:
                                    run_create_policy_assignment_method = (
                                        True if not old_other_leave_types else False
                                    )
                            ...
                            for old_other_leave_type in old_other_leave_types:
                                ...
                                leave_allocation_exists = frappe.db.get_all(
                                    "Leave Allocation",
                                    {
                                        "employee": doc.name,
                                        "leave_policy": old_leave_policy,
                                        "leave_type": old_other_leave_type.get(
                                            "leave_type"
                                        ),
                                        "from_date": ["<=", getdate()],
                                        "to_date": [">=", getdate()],
                            ...
                            for new_earned_leave_type in new_earned_leave_types:
                                ...
                                if is_calculate_leave_allocation:
                                    new_allocated_leaves = new_earned_leave_type.get(
                                        "annual_allocation"
                                    )
                                    frappe.log_error(
                                        "new_allocated_leaves", new_allocated_leaves
                                    )

                                    new_monthly_allocated_leaves = 0.0

                                    if new_allocated_leaves:
                                        new_monthly_allocated_leaves = (
                                            float(new_allocated_leaves) / 12
                                        )

                                    for old_earned_leave_type in old_earned_leave_types:
                                        old_monthly_allocated_leaves = 0.0

                                        if new_earned_leave_type.get(
                                            "leave_type"
                                        ) == old_earned_leave_type.get("leave_type"):
                                            old_allocated_leaves = (
                                                old_earned_leave_type.get(
                                                    "annual_allocation"
                                                )
                                            )
                                            frappe.log_error(
                                            ...
                                            if old_allocated_leaves:
                                                old_monthly_allocated_leaves = (
                                                    float(old_allocated_leaves) / 12
                                                )
                                                frappe.log_error(
                                                    "old_monthly_allocated_leaves",
                                                    old_monthly_allocated_leaves,
                                                ...
                                                if (
                                                    ...
                                                    final_leaves_to_allocate = float(
                                                        (
                                                            new_monthly_allocated_leaves
                                                            - old_monthly_allocated_leaves
                                                        )
                                ...
                                if not run_create_policy_assignment_method:
                                    create_leave_allocation(
                                        doc.name,
                                        doc.custom_leave_policy,
                                        confirmation_date,
                                        new_earned_leave_type.get("leave_type"),
                                        leave_allocation_to_date,
                                        final_leaves_to_allocate,
                                    )
                ...
                frappe.throw(
                    _(
                        "Cannot assign leave policy as there is no active Leave Period for the current date."
                    )
                )
...
def calculate_leave_allocation_based_on_confirmation(confirmation_date):
    ...
    if (
        confirmation_date.year == current_year
        and confirmation_date.month == current_month
    ):
    ...
    return {
        "calculate_leave_allocation": calculate_leave_allocation,
...
def get_policy_leave_types(leave_policy):
    try:
        leave_types = {"earned_leave_types": [], "other_leave_types": []}

        policy_leave_types = frappe.db.get_all(
            "Leave Policy Detail",
            {"parenttype": "Leave Policy", "parent": leave_policy},
        ...
        if policy_leave_types:
            for policy_leave_type in policy_leave_types:
                if policy_leave_type.get("leave_type"):
                    earned_leaves = frappe.db.get_value(
                        "Leave Type",
                        policy_leave_type.get("leave_type"),
                        ["custom_is_earned_leave_allocation", "is_earned_leave"],
                        as_dict=True,
                    )

                    if earned_leaves and (
                        earned_leaves.get("custom_is_earned_leave_allocation")
                        and earned_leaves.get("is_earned_leave")
                    ):
...
def create_leave_allocation(
    ...
    leave_policy_id,
    confirmation_date,
    prev_leave_allocation_leave_type,
    prev_leave_allocation_to_date,
    final_leaves_to_allocate=0.0,
):
    try:
...
def validate(doc, method):
    ...
    if doc.custom_weeklyoff and doc.custom_festival_holiday_list and not doc.holiday_list:
        ...
        for row in doc.custom_probation_extension_details:
            if not row.extended_date and (
                row.probation_end_date and row.extended_period
            ):
                row.extended_date = getdate(
                    add_to_date(row.probation_end_date, days=row.extended_period)
                )
    ...
    try:
        ...
        for weeklyoff_day in weeklyoff_days:
            ...
            if len(parts) == 1:
                ...
                reference_date = start_date + relativedelta.relativedelta(
                    weekday=weekday
                )

                while reference_date <= end_date:
                    if not any(
                        hd.get("date") == reference_date for hd in final_date_list
                    ):
                        final_date_list.append(
                            {
                                "date": reference_date,
                ...
                while current <= end_date:
                    nth_weekday_date = get_nth_weekday_of_month(
                        current.year, current.month, weekday, nth
                    )
                    if nth_weekday_date and start_date <= nth_weekday_date <= end_date:
                        if not any(
                            hd.get("date") == nth_weekday_date for hd in final_date_list
                        ):
                            final_date_list.append(
                                {
...
def after_insert(doc, method=None):
    ...
    candidate_portal = frappe.get_all(
        "Candidate Portal",
        filters={"applicant_email": doc.job_applicant, "company": doc.company},
        fields=["name"],
    ...
    final_doc_list.sort(
        key=lambda x: (
            x.type_of_document is None or x.type_of_document == "",
            x.type_of_document or "",
        )
    )
...
def create_shift_assignment(doc):
    ...
    if doc.default_shift and not frappe.db.exists(
        "Shift Assignment", {"employee": doc.name, "shift_type": doc.default_shift}
    ):
...
def create_leave_policy_assignment(
    employee_doc, based_on_joining_date, leave_period=None
):
    ...
    if doc.assignment_based_on == "Joining Date" and doc.employee:
        ...
        employee_joining_date = frappe.db.get_value(
            "Employee", doc.employee, "date_of_joining"
        )
    ...
    # ? SET LEAVE PERIOD IF AVAILABLE
        ...
        if (
            employee_doc.custom_leave_policy_assignment_from_date
            and employee_doc.custom_leave_policy_assignment_to_date
        ):
...
def validate_create_checkin_role(doc):
    ...
    if not doc.user_id:
        if not doc.is_new():
            ...
            if (
                doc_before_save.custom_attendance_capture_scheme
                != doc.custom_attendance_capture_scheme
            ):
...
def custom_autoname_employee(doc, method=None):
    ...
    if doc.naming_series in ["PE.####", "PI.####", "PC.####"]:
        ...
        existing_ids = frappe.db.get_all(
            "Employee", filters={"name": ["like", f"{prefix}%"]}, pluck="name"
        )
...
def check_if_employee_create_checkin_is_validate_via_web(user_id):
    ...
    try:
        ...
        capture_scheme = frappe.db.get_value(
            "Employee", {"user_id": user_id}, "custom_attendance_capture_scheme"
        )
                        ...
                        for i, row_data in enumerate(table_data, start=1):
                            ...
                            for k, v in row_data.items():
                                ...
                                label = frappe.db.get_value(
                                    "DocField",
                                    {"parent": response.get("options"), "fieldname": k},
                                    "label",
                                )
                                if not label:
                                    label = frappe.db.get_value(
                                        "Custom Field",
                                        {"dt": response.get("options"), "fieldname": k},
                                        "label",
                                    )

                                flat_row[k] = (
                                    {  # ? STORE BOTH FIELDNAME AND LABEL/VALUE
                                        "label": label or k,
                                        "value": v,
                                    }
                                )
...
def check_web_form_validation(user_id):
    try:
        ...
        employee = frappe.db.get_value(
            "Employee",
            {"user_id": user_id},
            ["name", "custom_employees_all_response_approve"],
            as_dict=True,
        )
        # ? FIND IF PRE LOGIN QUESTIONNAIRE TABLE EXISTS OR NOT FOR THAT EMPLOYEE
        employee_pre_login_responses = frappe.get_all(
            "Pre Login Questionnaire Response",
        ...
        if not hr_settings.custom_pre_login_questionnaire:
            return {
                "success": 1,
                "message": "No questionnaire Set in HR Settings",
                "data": True,
            }
        ...
        if not hr_settings.custom_enable_welcome_page_for_prompt:
            return {
                "success": 1,
                "message": "Welcome Page not enabled for this company",
                "data": True,
            }
        ...
        if not employee_pre_login_responses:
            return {
                "success": 1,
                "message": "No questionnaire Needed",
                "data": employee.name,
            }
        ...
        if previous_status != "Confirmed" or state == "Confirmed":
            ...
            if doc.custom_leave_policy:
                policy_name = frappe.db.get_value(
                    "Leave Policy", doc.custom_leave_policy, "title"
                )
                ...
                if "probation" in policy_name.lower():
                    ...
                    clean_policy_name = re.sub(
                        r"[\s\-\+]*probation.*", "", policy_name, flags=re.IGNORECASE
                    ).strip()
                    if clean_policy_name:
                        # ? CHECK IF A POLICY EXISTS WITH THIS TITLE
                        new_policy = frappe.db.get_value(
                            "Leave Policy", {"title": clean_policy_name}, "name"
                        )
                        if new_policy:
...
def get_field_visibility_settings(employee=None, user=None):
    ...
    if field_visibility_settings:
        employee_data = frappe.get_all(
            "Employee", {"employee": employee}, field_visibility_settings
        )[0]
...
# ? ENQUEUE FUNCTION
# ! prompt_hr.py.employee.rename_selected_employees
@frappe.whitelist()
def rename_selected_employees(employee_list, max_minutes=30):
    """
