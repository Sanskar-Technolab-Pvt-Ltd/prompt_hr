...
def update_amount_in_marketing_planning(doc,method):
    ...
    if doc.campaign:
        ...
        if campaign_doc.custom_marketing_plan:
            ...
            marketing_plan = frappe.get_doc("Marketing Planning",campaign_doc.custom_marketing_plan)
            ...
            if method == "before_submit":
                ...
                budget_exceeded = is_budget_exceeded(campaign_doc, marketing_plan, current_quarter, doc.total)
                ...
                if budget_exceeded and control_action:
                    enforce_budget_control(campaign_doc.custom_campaign_type,current_quarter,marketing_plan,control_action)

                # ? UPDATE MARKETING PLAN EXPENSE AND REMAINING BUDGET VALUES
                update_marketing_planning_row(doc,method, campaign_doc, marketing_plan, current_quarter)
...
def get_control_flags(doc, marketing_plan):
    ...
    if doc.doctype == "Purchase Invoice":
        ...
        control_action = marketing_plan.action_if_budget_exceeded_on_pi if control_enabled else None

    # ? FOR EXPENSE CLAIM
    elif doc.doctype == "Expense Claim":
        control_enabled = marketing_plan.applicable_on_expense_claim
        control_action = marketing_plan.action_if_budget_exceeded_on_ec if control_enabled else None
...
def is_budget_exceeded(campaign_doc, marketing_plan, current_quarter, invoice_total):
    for row in marketing_plan.monthly_campaign_planning:
        if row.campaign_type == campaign_doc.custom_campaign_type and row.month == current_quarter:
...
def update_marketing_planning_row(doc,method, campaign_doc, marketing_plan, current_quarter):
    for row in marketing_plan.monthly_campaign_planning:
    ...
    for q_row in marketing_plan.quarter_budget_planning:
        ...
        if q_row.quarter == current_quarter:
            ...
            total_actual_expense = sum(
                r.actual_expense for r in marketing_plan.monthly_campaign_planning if r.month == current_quarter
            )
...
def quarter_map(date):
    ...
    current_quarter = next((quarter for quarter,months in quarter_map.items() if start_month in months),None)
...
def record_expense_log_in_campaign(doc, campaign_doc, method):
    ...
    if method == "before_submit":
        ...
        campaign_doc.append("custom_logs", {
        ...
        logs_to_remove = [row for row in campaign_doc.custom_logs if row.id == doc.name and row.entry_type == entry_type]
