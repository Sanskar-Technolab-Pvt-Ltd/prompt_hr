...
def get_fields(employee=None):
    try:
        ...
        if not parentfield:
            frappe.throw("There are currently no personal details you're allowed to update. Please contact HR.")
...
@frappe.whitelist()
def create_field_changes(employee=None, field_name=None, field_label=None, new_value =None, old_value=None):
    try:
        
        if not employee or not field_name or not field_label or new_value is None:
            frappe.throw("Employee, field name, field label, new value and old value are required.")
