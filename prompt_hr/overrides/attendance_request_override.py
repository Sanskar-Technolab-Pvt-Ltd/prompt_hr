...
class CustomAttendanceRequest(AttendanceRequest):
    ...
    def before_submit(self):
        if self.workflow_state == "Approved":
            ...
            frappe.enqueue(
                self.process_attendance_and_penalties,
                queue="long",
                doc=self.as_dict(),
                now=False,
            )
            ...
            #! HANDLE REJECTION ALSO IN BACKGROUND
            frappe.enqueue(
                self.process_rejection_penalties,
                queue="long",
                doc=self.as_dict(),
                now=False,
            )
    ...
    # ===================================================================
    # BACKGROUND JOB: APPROVED
    # ===================================================================
    @staticmethod
    def process_attendance_and_penalties(doc):
        from_date = getdate(doc["from_date"])
        to_date = min(getdate(doc["to_date"]), getdate("2025-10-05"))
