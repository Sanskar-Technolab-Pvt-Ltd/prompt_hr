...
class ExitApprovalProcess(Document):
    def on_update(self):
        ...
        if (
            self.resignation_approval == "Approved"
            and frappe.db.get_value("Employee", self.employee, "relieving_date")
    ...
    try:
        ...
        if employee_separation_template:
            ...
            activities = frappe.get_all(
                ...
                fields=[
                    "activity_name",
                    "custom_checklist_name",
                    "custom_checklist_record",
                    "role",
                    "user",
                    "custom_is_submitted",
                    "custom_is_raised",
        ...
        frappe.log_error(
            frappe.get_traceback(), "Validation Error in raise_exit_checklist"
        )
        return {
            "status": "error",
    ...
    try:
        ...
        if company == get_prompt_company_name().get("company_name"):
            quiz_name = frappe.db.get_value(
                "HR Settings", None, "custom_exit_interview_quiz_prompt"
            )
        elif company == get_indifoss_company_name().get("company_name"):
            quiz_name = frappe.db.get_value(
                "HR Settings", None, "custom_exit_interview_quiz_indifoss"
            )
        else:
...
def send_exit_interview_notification(employee, exit_interview_name):
    try:
        ...
        frappe.log_error(
            frappe.get_traceback(), "Error in send_exit_interview_notification"
        )
        return {
            "status": "error",
