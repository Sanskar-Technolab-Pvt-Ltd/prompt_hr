...
class AttendanceRegularization(Document):
    ...
    def validate(self):
        ...
        if get_datetime(self.regularization_date).date() >= getdate():
            frappe.throw(
                "Attendance Regularization cannot be raised for future or current dates."
            )
        ...
        is_rh = is_user_reporting_manager_or_hr(
            user_id=frappe.session.user, requesting_employee_id=self.employee
        )
        if is_rh:
            if self.status == "Approved":

                in_times = [
                    get_datetime(f"{self.regularization_date} {row.in_time}")
                    for row in self.checkinpunch_details
                    if row.in_time
                ]
                out_times = [
                    get_datetime(f"{self.regularization_date} {row.out_time}")
                    for row in self.checkinpunch_details
                    if row.out_time
                ]
                ...
                if not self.employee_notified:
                    ...
                    if emp_user_id:
                        ...
                        if not self.is_new():
                            auto_approve = frappe.db.get_value(
                                "Attendance Regularization", self.name, "auto_approve"
                            )
                            if auto_approve:
                                is_email_sent_allowed = (
                                    frappe.db.get_single_value(
                                        "HR Settings",
                                        "custom_send_auto_approve_doc_emails",
                                    )
                                    or 0
                                )
                ...
                if emp_user_id:
                    ...
                    if not self.is_new():
                        ...
                        if auto_approve:
                            is_email_sent_allowed = (
                                frappe.db.get_single_value(
                                    "HR Settings", "custom_send_auto_approve_doc_emails"
                                )
                                or 0
                            )
                            if not is_email_sent_allowed:
    ...
    def before_save(self):
        ...
        attendance_regularization_limit = int(
            frappe.db.get_single_value(
                "HR Settings",
                "custom_allowed_to_raise_regularizations_for_past_days_for_prompt",
            )
            or 0
        )
