...
frappe.query_reports["Leave Balance"] = {
    ...
    onload: () => {
        ...
        // First set from/to date using your existing call
        frappe.call({
            type: "GET",
        ...
        // Now fetch and set the employee linked to the current user
        frappe.call({
            ...
            callback: (r) => {
                if (r.message && r.message.name) {
                    // Set the employee filter value
                    frappe.query_report.set_filter_value("employee", r.message.name);
                }
            }
        });
    }
