...
def penalize_prompt_employee():
    ...
    try:
        ...
        allowed_late_entries = (
            hr_settings_doc.custom_late_coming_allowed_per_month_for_prompt
        )

        # * FOR LATE COMING
        late_entry_leave_type = hr_settings_doc.custom_leave_type_for_prompt
        late_entry_leave_deduction = (
            hr_settings_doc.custom_deduction_of_leave_for_prompt
        )
        is_lwp_for_late_entry = (
            1
            if hr_settings_doc.custom_deduct_leave_penalty_for_prompt
            == "Deduct leave without pay"
            else 0
        )

        late_entry_deduct_leave = (
            0.5 if late_entry_leave_deduction == "Half day" else 1.0
        )

        leave_penalty_buffer_days = (
            hr_settings_doc.custom_buffer_period_for_leave_penalty_for_prompt
        )
        ...
        if leave_penalty_buffer_days:
            ...
            print("Check Attendance Date 1\n\n", leave_penalty_check_attendance_date)
        else:
            leave_penalty_check_attendance_date = getdate(today())
            print("Check Attendance Date 2\n\n", leave_penalty_check_attendance_date)

        # * FOR DAILY HOURS
        expected_work_hours = (
            hr_settings_doc.custom_daily_hours_criteria_for_penalty_for_prompt
        )
        daily_hours_leave_type = hr_settings_doc.custom_leave_type_daily_for_prompt
        daily_hours_leave_deduction = (
            hr_settings_doc.custom_deduction_of_leave_daily_for_prompt
        )
        working_hours_buffer_days = (
            hr_settings_doc.custom_buffer_period_for_daily_hours_penalty_for_prompt
        )

        insufficient_hours_deduct_leave = (
            0.5 if daily_hours_leave_deduction == "Half day" else 1.0
        )

        is_lwp_for_insufficient_hours = (
            1
            if hr_settings_doc.custom_deduct_leave_penalty_daily_for_prompt
            == "Deduct leave without pay"
            else 0
        )
        ...
        no_attendance_buffer_days = (
            hr_settings_doc.custom_buffer_period_for_no_attendance_penalty_for_prompt
        )
        ...
        if not company_id.get("error") and company_id.get("company_id"):
            ...
            if prompt_employee_list:
                ...
                for emp_id in prompt_employee_list:
                    ...
                    employee_scheduled_for_late_entry_penalty = (
                        penalize_employee_for_late_entry_for_prompt(
                            emp_id,
                    ...
                    if employee_scheduled_for_late_entry_penalty:
                        send_penalty_warnings(
                            employee_scheduled_for_late_entry_penalty, "Late Entry"
                        )
                    ...
                    employee_scheduled_for_incomplete_day_penalty = (
                        penalize_incomplete_day_for_prompt(
                            emp_id,
                    ...
                    if employee_scheduled_for_no_attendance_penalty:
                        send_penalty_warnings(
                            employee_scheduled_for_no_attendance_penalty,
                            "No Attendance",
                        )
...
def penalize_employee_for_late_entry_for_prompt(
    ...
    try:
        # DEBUG 1: Employee ID and Check for Eligibility
        print(f"\n\n[DEBUG] Checking penalty criteria for: {emp_id.get('name')}")
        if not check_employee_penalty_criteria(emp_id.get("name"), "For Late Arrival"):
            print(f"[DEBUG] {emp_id.get('name')} is NOT eligible for late entry penalty.")
            return
        print(f"[DEBUG] {emp_id.get('name')} is eligible for late entry penalty.")

        # DEBUG 2: Fetch All Valid Late Attendance Entries
        late_attendance_list = frappe.db.get_all(
            "Attendance",
            {
                "docstatus": 1,
                "employee": emp_id.get("name"),
        ...
        print(f"[DEBUG] Late attendance entries found: {len(late_attendance_list)}")
        print(f"[DEBUG] Allowed late entries before penalty: {allowed_late_entries}")

        # DEBUG 3: Leave Balance Calculations
        leave_allocation_id = None
        remaining_leave_balance_for_late_entry = get_remaining_leaves(
            late_entry_leave_type, emp_id.get("name"), company_id
        )
        print(f"[DEBUG] Remaining leave balance: {remaining_leave_balance_for_late_entry}")
        print(f"[DEBUG] Is LWP for late entry? {is_lwp_for_late_entry}")

        if not is_lwp_for_late_entry:
            if remaining_leave_balance_for_late_entry <= 0:
                earned_leave = 0.0
            ...
            print(f"[DEBUG] Earned Leave: {earned_leave}, LWP Leave: {lwp_leave}")
            print(f"[DEBUG] Leave Allocation ID: {leave_allocation_id}")
        else:
            earned_leave = 0.0
            lwp_leave = late_entry_deduct_leave
            print(f"[DEBUG] Entire penalty as LWP - LWP Leave: {lwp_leave}")

        # DEBUG 4: Begin Penalty Evaluation
        if late_attendance_list:
            ...
            print(f"[DEBUG] Entries to penalize: {len(penalizable_entries)}")

            for attendance_id in penalizable_entries:
                print(f"[DEBUG] Evaluating late entry: {attendance_id}")
                ...
                leave_application_exists = frappe.db.exists(
                    "Leave Application",
                    {
                        "employee": emp_id.get("name"),
                        "docstatus": 1,
                        "from_date": ["<=", attendance_id.get("attendance_date")],
                        "to_date": [">=", attendance_id.get("attendance_date")],
                    },
                )
                ...
                attendance_regularization_exists = frappe.db.get_all(
                    ...
                    {
                        "employee": emp_id.get("name"),
                        "attendance": attendance_id.get("name"),
                        "regularization_date": attendance_id.get("attendance_date"),
                    },
                    "name",
                )

                employee_late_penalty_exists = frappe.db.exists(
                    "Employee Penalty",
                    {
                        "employee": emp_id.get("name"),
                        "attendance": attendance_id.get("name"),
                ...
                print(f"[DEBUG] Leave Exists: {bool(leave_application_exists)}, Regularization Exists: {bool(attendance_regularization_exists)}, Penalty Already Exists: {bool(employee_late_penalty_exists)}")

                # If no leave/regularization/penalty exists and date is valid
                if (
                    not (
                        leave_application_exists
                        or attendance_regularization_exists
                        or employee_late_penalty_exists
                    )
                    and getdate(attendance_id.get("attendance_date")) <= check_attendance_date
                ):
                    print(
                        f"\n[DEBUG] Creating Penalty for {emp_id.get('name')} on {attendance_id.get('attendance_date')}"
                    )
                ...
                # Penalty to be created tomorrow
                if (
                    not (
                        leave_application_exists
                        or attendance_regularization_exists
                    ...
                    and getdate(attendance_id.get("attendance_date"))
                    == add_to_date(check_attendance_date, days=1)
                ):
                    print(f"[DEBUG] Penalty scheduled for tomorrow: {emp_id.get('name')}")
                    employee_scheduled_for_penalty_tomorrow = emp_id.get("name")

            return employee_scheduled_for_penalty_tomorrow

        print(f"[DEBUG] No late entries beyond threshold for: {emp_id.get('name')}")
        return None
...
def penalize_incomplete_day_for_prompt(
    ...
    try:
        ...
        attendance_list = [
            ...
            if 0 < a.working_hours < expected_work_hours
        ]


        if attendance_list:
            employee_scheduled_for_penalty_tomorrow = None
            remaining_leave_balance = get_remaining_leaves(
                daily_hours_leave_type, emp.get("name"), company_id
            )
            if not is_lwp_for_insufficient_hours:
                ...
                if not remaining_leave_balance > 0:
                    ...
                    if remaining_leave_balance >= insufficient_hours_deduct_leave:
                        ...
                        lwp_leave = (
                            insufficient_hours_deduct_leave - remaining_leave_balance
                        )
                    leave_allocation_id = frappe.db.get_value(
                        "Leave Allocation",
                        {
            ...
            for attendance in attendance_list:
                ...
                if (
                    ...
                    and getdate(attendance.get("attendance_date"))
                    <= check_attendance_date
                ):
                    create_employee_penalty(
                        emp.get("name"),
                ...
                if not (
                    ...
                    or attendance_regularization_exists
                    or employee_late_penalty_exists
                ) and getdate(attendance.get("attendance_date")) == add_to_date(
                    check_attendance_date, days=1
                ):
        ...
        frappe.log_error(
            "Error in penalize_incomplete_day scheduler", frappe.get_traceback()
        )
    ...
    try:
        ...
        if not frappe.db.exists(
            ...
            attendance_regularization_exists_next_day = frappe.db.exists(
                "Attendance Regularization",
                {"employee": emp.get("name"), "regularization_date": next_day},
                "name",
            )
...
def create_employee_penalty(
        ...
        if company_id and employee.company == company_id:
            ...
            if notification:
                if not employee.user_id:
                    frappe.log_error(
                        f"User ID not set for Employee {employee.employee_name}"
                    )
                    return
    ...
    if (
        not (
            is_lwp_for_late_entry
            or is_lwp_for_insufficient_hours
            or is_lwp_for_no_attendance
        )
        and earned_leave > 0
    ):
...
def penalize_employee_for_attendance_mispunch_prompt(args):
    ...
    print(f"[DEBUG] Starting penalization for employee: {emp}\n\n")
    ...
    try:
        ...
        print(
            f"[DEBUG] Found {len(attendance_list)} mispunch attendance(s): {[i.name for i in attendance_list]}"
        )
        ...
        if attendance_list:
            ...
            existing_penalty = frappe.db.exists(
                "Employee Penalty",
                {
                    "employee": emp.get("name"),  # Replace with emp if needed
                    "attendance": ["in", [i.name for i in attendance_list]],
                    "for_mispunch": 1,
                },
            )
            print(
                f"[DEBUG] Existing penalty check: {'Found' if existing_penalty else 'Not found'}"
            )
        ...
        frappe.log_error(
            f"Error in penalize_employee_for_attendance_mispunch_prompt: {str(e)}"
        )
        print(f"[ERROR] Exception occurred: {str(e)}")
