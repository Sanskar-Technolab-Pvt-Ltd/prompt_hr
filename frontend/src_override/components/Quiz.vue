  ...
  <div v-if="quiz.data">
    ...
    >
      ...
      <div v-if="quiz.data?.duration" class="leading-5">
        {{
          __("Please ensure that you complete all the questions in {0} minutes.").format(
            quiz.data.duration
          )
        }}
      </div>
    ...
    <div v-if="activeQuestion == 0">
      <div class="border text-center p-20 rounded text-sm max-w-md mx-auto">
        <div class="font-semibold text-lg text-ink-gray-9 mb-4">
          {{ quiz.data.title }}
        </div>
        ...
        <div class="mb-2">
          <input
            v-model="phone"
            type="text"
            placeholder="Phone Numberrrrrr"
            class="w-5/6 px-3 py-2 border rounded-md"
          />
        </div>
        ...
        <!-- ✅ Both v-if and v-else are on the same level, no wrapping divs -->
        <Button
        ...
        <div v-else class="leading-5 text-ink-gray-7 mb-2">
          {{
            __(
    ...
    <div
      v-if="
        quiz.data.show_submission_history && attempts?.data && attempts.data.length > 0
      "
      class="mt-10"
    >
      <ListView
...
const startQuiz = async () => {
  ...
  try {
    ...
    // ? DISABLE PAGE REFRESH AND HARD REFRESH WARNINGS
    window.onbeforeunload = function (e) {
        e.preventDefault();
        e.returnValue = "Are you sure you want to leave? Your quiz progress may be lost.";
    };

    // ? PERFORM A FRAPPE CALL TO VALIDATE THE PHONE NUMBER AND PASSWORD
    const response = await call('prompt_hr.py.lms.start_quiz_rituals', {
      phone: phone.value,
      password: password.value,
      quiz_id: quiz.data.name,
    });
    // ? CHECK IF THE RESPONSE IS TRUE
    if (response==true) {
      // ? IF VALIDATION IS SUCCESSFUL, START THE QUIZ
      ...
      createToast({
        ...
        icon: "alert-circle",
        iconClasses: "text-red-600 bg-red-100 rounded-full",
        position: "top-center",
      });
    }
  } catch (error) {
